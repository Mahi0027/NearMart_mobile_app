...
import ShopFlatLists from '../elements/ShopFlatLists';
import ScanScreen from '../elements/ScanScreen';
...
const ShopHome = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (searchedText !== '') {
        await shopSearchScreen(searchedText);
      }
    })();
  });
  ...
  // searched shop list show
  const shopSearchScreen = text => {
    getData = [];
